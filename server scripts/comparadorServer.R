...
rank_bar_data <- reactive({
  ...
  rank_bar <- rank_bar %>%
    mutate(
      ...
      comp_value = if (use_standardized) rank_compar()$oe else rank_compar()$r,
...
output$select_ambit_ranquing_ui <- renderUI({
  #ambit <- sort(unique(dades_tbl$ambit[dades_tbl$`Centre/Territori` == selected_values$center]))
...
output$select_indicador_ranquing_ui <- renderUI({
  ...
  dades_comparador2 <- dades_tbl %>%
    distinct(nom_indicador, dimensio, ambit, 
             #tag1, tag2, tag3, 
             etiqueta,
             Granularitat, `Centre/Territori`) %>%
    filter(
      Granularitat == selected_values$geography,
      `Centre/Territori` == selected_values$center,
      ambit == selected_values$ambit |
...
output$selector_sexe_rank_ui <- renderUI({
  ...
  if (!has_standardized_results() || !isTRUE(input$result_toggle)) {
...
sexe_rank_exists <- reactive({
  ...
  !has_standardized_results() || !isTRUE(input$result_toggle)
})
...
output$titol_comparador <- renderUI({
  if (isTRUE(input$result_toggle)) {
    ...
    HTML(paste0("<strong>", input$select_indicador_ranquing, " (", unique(rank_bar_data()$unitats), ")", "</strong>"))
...
output$subtitol_comparador2 <- renderText({ 
  ...
  has_sexe <- !is.null(input$selector_sexe_rank) && length(input$selector_sexe_rank) > 0
  has_edat <- !is.null(input$selector_edat_rank) && length(input$selector_edat_rank) > 0
  
  if (sexe_rank_exists() && has_sexe) {
    
    if (edat_rank_exists() && has_edat) {
      
      # Hide sexe when it's "Total"
      if (input$selector_sexe_rank == "Total") {
        # Also hide edat when it's "Total"
        if (input$selector_edat_rank == "Total") {
        ...
        if (input$selector_edat_rank == "Total") {
          ...
          HTML(paste0("Sexe: ", input$selector_sexe_rank, ", Grup d'edat: ", input$selector_edat_rank))
        }
      }
  ...
  if (is.data.frame(rank_bar_data()) && nrow(rank_bar_data()) == 0) {
    ...
    tooltip_bar <- if (isTRUE(input$result_toggle)) {
      ...
      paste0(#r_rank_bar_data()$Granularitat, ": ", 
        rank_bar_data()$`Centre/Territori`, ": ", "<b>", round(rank_bar_data()$resultat, 2), "</b> (", rank_bar_data()$unitats, ")",
        "<br>", selected_values$center, ": ", "<b>", round(rank_bar_data()$comp_value, 2), "</b> (", rank_bar_data()$unitats, ")",
        "<br>", rank_bar_data$int_hover)
    ...
    if (nrow(rank_bar_data()) > 20) {
      ...
      rank_plot <- plot_ly(data = rank_bar_data, height = 400) %>%
        add_trace(x = ~`Centre/Territori`, 
                  ...
                  type = 'scatter', 
                  mode = 'lines+markers',
                  text = ~paste0(selected_values$center, ": <b>", round(comp_value, 2), "</b> (", unitats, ")"),
        ...
        layout(
          yaxis = list(title = list(text = unique(rank_bar_data()$unitats), standoff = 10), 
      ...
      rank_plot <- plot_ly(data = rank_bar_data, height = plot_height) %>%
        ...
        layout(
          separators = ",.",     
          xaxis = list(title = list(text = unique(rank_bar_data()$unitats), standoff = 10), 
                       titlefont = list(size = 14), 
                       tickfont = list(size = 14, family = "sans-serif"), fixedrange = TRUE),
...
output$rank_table <- DT::renderDataTable({
  ...
  data <- rank_bar_data()
  result_col <- if (isTRUE(input$result_toggle)) "oe" else "r"
