...
output$select_ambit_analisi_ui <- renderUI({
  ...
  ambit <- dades_ambit_ind_tbl %>%
    pull(ambit) %>%
    as.character() %>%
    unique() %>%
    sort()
  
  etiqueta <- dades_ambit_ind_tbl %>%
...
output$select_indicador_analisi_ui <- renderUI({
  ...
  filtered_data <- dades_ambit_ind_tbl %>%
    distinct(id_indicador, indicador, dimensio, ambit, tag1, tag2, tag3) %>%
    filter(
...
indicador_id_reactive <- reactive({
  ...
  id_data <- dades_ambit_ind_tbl %>%
    filter(indicador == r_selected_values$indicador) %>%
    distinct(id_indicador) %>%
    collect()
  ...
  if(nrow(id_data) > 0 && !is.na(id_data$id_indicador[1])) {
    return(id_data$id_indicador[1])
  } else {
    return(NULL)
  }
})
